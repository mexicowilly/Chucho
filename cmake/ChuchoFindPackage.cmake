...
FUNCTION(CHUCHO_FIND_PACKAGE CHUCHO_PKG_NAME)
    ...
    IF(${SHOULD_FIND_${CHUCHO_PKG_NAME}} EQUAL -1)
        RETURN()
    ENDIF()
    LIST(FIND LOWER_WITH_LIBS ${LOWER_PKG_NAME} BUILT_IN_PKG)
    IF (${BUILT_IN_PKG} EQUAL -1)
        ...
        ENDIF()
        IF(LIB_ARGS)
            MESSAGE(STATUS "Looking for ${LIB_ARGS}")
            FIND_LIBRARY(${CHUCHO_PKG_NAME}_LIB NAMES ${LIB_ARGS} HINTS ${${CHUCHO_PKG_NAME}_PKGCNF_LIBRARY_DIRS} ${${CHUCHO_PKG_NAME}_LIB})
            IF(${CHUCHO_PKG_NAME}_LIB)
                MESSAGE(STATUS "Looking for ${LIB_ARGS} - ${${CHUCHO_PKG_NAME}_LIB}")
            ELSE()
        ...
        SET(CMAKE_REQUIRED_LIBRARIES ${${CHUCHO_PKG_NAME}_LIB})
        CHUCHO_REQUIRE_SYMBOLS(${INCLUDE_ARGS} ${SYMBOL_ARGS})
        UNSET(CMAKE_REQUIRED_INCLUDES)
        UNSET(CMAKE_REQUIRED_LIBRARIES)
        SET(${CHUCHO_PKG_NAME}_INCLUDE_DIR ${${CHUCHO_PKG_NAME}_INCLUDE_DIR} PARENT_SCOPE)
        SET(${CHUCHO_PKG_NAME}_LIB ${${CHUCHO_PKG_NAME}_LIB} PARENT_SCOPE)
        SET(CHUCHO_HAVE_${CHUCHO_PKG_NAME} TRUE PARENT_SCOPE)
        MARK_AS_ADVANCED(${CHUCHO_PKG_NAME}_INCLUDE_DIR ${CHUCHO_PKG_NAME}_LIB CHUCHO_HAVE_${CHUCHO_PKG_NAME})
    ELSE()
