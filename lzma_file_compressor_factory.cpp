...
std::shared_ptr<configurable> lzma_file_compressor_factory::create_configurable(std::shared_ptr<memento> mnto)
{
#if defined(CHUCHO_HAVE_LZMA)
    ...
    auto gfc = std::make_shared<lzma_file_compressor>(mi);
#else
    report_warning("A lzma_file_compressor was requested in the configuration, but this Chucho library was built without lzma support.");
