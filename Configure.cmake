    ...
    IF(CHUCHO_SQLITE_H)
        IF(SQLITE_CLIENT_LIB)
            ...
            FOREACH(SYM sqlite3_threadsafe sqlite3_open sqlite3_prepare_v2 sqlite3_reset sqlite3_bind_text
                ...
                IF(NOT CHUCHO_HAVE_${SYM})
                    MESSAGE(FATAL_ERROR "${SQLITE_CLIENT_LIB} does not contain expected SQLite symbols: ${SYM}")
                ENDIF()
            ENDFOREACH()
        ELSEIF(ENABLE_SHARED)
...
IF(POSTGRES_INCLUDE_DIR)
    ...
    IF(CHUCHO_POSTGRES_H)
        IF(POSTGRES_CLIENT_LIB)
            ...
            FOREACH(SYM PQconnectdb PQprepare PQexecPrepared PQresultStatus PQresultErrorMessage PQclear PQfinish
                        PQstatus PQerrorMessage)
                CHECK_CXX_SYMBOL_EXISTS(${SYM} libpq-fe.h CHUCHO_HAVE_${SYM})
                IF(NOT CHUCHO_HAVE_${SYM})
                    MESSAGE(FATAL_ERROR "${POSTGRES_CLIENT_LIB} does not contain expected PostgreSQL symbols: ${SYM}")
                ENDIF()
            ENDFOREACH()
        ELSEIF(ENABLE_SHARED)
