...
ELSEIF(CMAKE_COMPILER_IS_GNUCXX)
    ...
    ENDIF()
    SET(CHUCHO_STD_FLAG -std=c++11)
    CHECK_CXX_COMPILER_FLAG(${CHUCHO_STD_FLAG} CHUCHO_HAVE_STD)
    IF(CHUCHO_HAVE_STD)
        SET(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${CHUCHO_STD_FLAG}")
    ELSE()
        MESSAGE(FATAL_ERROR "${CHUCHO_STD_FLAG} is required")
    ENDIF()
    IF(ENABLE_SHARED)
        CHECK_CXX_COMPILER_FLAG(-fvisibility=hidden CHUCHO_VIS_FLAG)
        IF(CHUCHO_VIS_FLAG)
...
IF(CAPN_PROTO_INCLUDE_DIR AND CAPN_PROTO_LIB AND CAPN_PROTO_KJ_LIB AND CAPNP_DIR)
    ...
    CHECK_INCLUDE_FILE_CXX(capnp/message.h CHUCHO_CAPN_PROTO_MESSAGE_H)
    CHECK_INCLUDE_FILE_CXX(capnp/serialize-packed.h CHUCHO_CAPN_PROTO_SERIALIZE_PACKED_H)
    CHECK_INCLUDE_FILE_CXX(kj/vector.h CHUCHO_CAPN_PROTO_VECTOR_H)
