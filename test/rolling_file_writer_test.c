...
static void rolling_file_writer_test(void)
{
    /*
    ...
    chucho_writer* wrt;
    rc = chucho_create_rolling_file_writer(&wrt, fmt, "hello", CHUCHO_ON_START_APPEND, 0);
...
void run_rolling_file_writer_test(void)
{
    ...
    sput_run_test(rolling_file_writer_test);
    sput_leave_suite();
}
